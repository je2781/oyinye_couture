    ...
    return <main className="container mx-auto bg-white w-full min-h-screen max-w-7xl md:py-12 py-8 md:px-12 px-8 flex flex-col gap-y-9 items-center">
        <article className="space-y-11">
            ...
            <form className="space-y-5 text-gray-500 font-sans text-sm" onSubmit={handleFormSubmit}>
                ...
                <div className="w-full flex flex-col gap-y-2">
                    <div className="w-full flex flex-col gap-y-2">
                        <label htmlFor="phone" className="tracking-widest">PHONE NUMBER</label>
                        <input 
                    ...
                    <p id='phone-error' className="text-red-600 text-xs font-sans hidden -mt-1">A phone number is required</p>
                </div>
                <div className="w-full flex flex-col gap-y-2">
                    <div className="w-full flex flex-col gap-y-2">
                        <label htmlFor="event-date" className="tracking-widest">DATE OF EVENT?</label>
                        <input 
                            onChange={(e) => setEventDate(e.target.value)}
                            value={eventDate}
                            onBlur={(e) => {
                                if(e.target.value.length === 0){
                                    document.querySelector('#event-date-error')?.classList.remove('hidden');
                                }else{
                                    document.querySelector('#event-date-error')?.classList.add('hidden');
                                }
                            }} 
                            type="date" 
                            id='event-date' 
                    ...
                    <p id='event-date-error' className="text-red-600 text-xs font-sans hidden -mt-1">An event date is required</p>
                </div>
                <div className="w-full flex flex-col gap-y-2">
                    <div className="w-full flex flex-col gap-y-2">
                        <label htmlFor="size" className="tracking-widest">YOUR STANDARD SIZE (UK/US)</label>
                        <select 
                            onChange={(e) => setSize(e.target.value)}
                            onBlur={(e) => {
                                if(e.target.value.length === 0){
                                    document.querySelector('#size-error')?.classList.remove('hidden');
                                }else{
                                    document.querySelector('#size-error')?.classList.add('hidden');
                                }
                            }} 
                            id='size' 
                    ...
                    <p id='size-error' className="text-red-600 text-xs font-sans hidden -mt-1">A standard size is required</p>
                </div>
                <div className="w-full flex flex-col gap-y-2 ">
                    <label htmlFor="country" className="tracking-widest">COUNTRY</label>
                    <select
                      id='country'
                      onChange={(e) => {
                    ...
                    <div className="flex flex-row gap-x-3">
                        <div className="inline-flex flex-row gap-x-2 items-center">
                            ...
                            onChange={() => {
                                ...
                                (document.getElementById('no') as HTMLInputElement).checked = false;
                ...
                <div className="flex-col w-full hidden gap-y-3" id='upload-container'>
                    <div className="flex flex-col items-start">
                        {uploadedFiles.map((fileName: string, index: number) => <p key={index}>{fileName}</p>)}
                    </div>
                    <div className="flex-col flex w-full">
                        ...
                        >
                            <i className="fa-solid fa-camera text-lg"></i><span>Upload Image(s)</span>
                        </label>
