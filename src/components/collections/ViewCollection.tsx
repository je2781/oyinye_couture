  ...
  return (
    <div className="flex flex-col gap-y-8 bg-white max-w-7xl w-full lg:py-12 py-8 mx-auto px-8 container">
      ...
      <section className="flex flex-col lg:items-start items-center gap-y-5 font-sans">
        <header className="lg:text-4xl text-3xl font-normal">
        ...
        <div className="flex flex-row items-center justify-evenly flex-wrap gap-x-2 gap-y-4 leading-text">
          {featuredProducts.filter((product: any) => product.isFeature === true).slice(0, 4).map((product: any, i: number) => <Product key={i} product={product} isOnHomePage={true}/>)}
        </div>
