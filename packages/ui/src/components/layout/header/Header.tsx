...
export default function Header({ cartItems, isCheckout, isAuth }: any) {
  ...
  useEffect(() => {
    // Only update if cartItems have actually changed
    if (
      cartItems &&
  ...
  return (
    <nav
      className={
        isCheckout
          ? "w-full h-[96px] bg-white border border-gray-300 border-t-0 border-l-0 border-r-0"
          : `z-20 md:shadow-md fixed top-0 w-full h-[96px] bg-white ${
    ...
    >
      <div className="flex items-center flex-row justify-between md:py-5 container mx-auto px-4 lg:pl-0 lg:pr-6 h-full">
        <button
          id="toggle-button"
            ...
            {menuItems.map((item) => (
              <li key={item.name} className={`cursor-pointer relative`}>
                {item.name !== "Collections" ? (
                  ...
                  <div
                    ...
                    onClick={(e) => {
                      ...
                      let collectionsDropdown = document.getElementById(
                ...
                <ul
                  ...
                  className="hide hidden absolute right-0 z-10 top-8 left-[75px] w-36 origin-top-right flex-col p-1 text-sm bg-white rounded-md text-gray-600 font-light shadow-sm shadow-gray-400"
                >
                  {["All Dresses", "Basics", "Made To Order", "Bespoke"].map(
                    (item: string, _) => (
                      <li
                        key={item}
