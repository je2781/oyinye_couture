...
export default function CartInfo({ total, cartItems, userEmail, csrf }: any) {
  ...
  const [quantities, setQuantities] = useState<number[]>(
    Object.values(cartItemObj.current!).map((item: any) => item.quantity)
  );
  const [itemTotalAmounts, setItemTotalAmounts] = useState<number[]>(
    Object.values(cartItemObj.current!).map(
        ...
        <main className="min-h-screen w-full container mx-auto md:pl-16 px-8 md:pt-12 pt-5 pb-9 flex flex-col gap-y-9">
          <header className="flex md:flex-row flex-col gap-y-5 justify-between items-start md:items-center pt-6 w-full">
            ...
            <div className="inline-flex flex-col gap-y-1 md:w-[40%] w-full text-gray-600">
              ...
              <div
                id="email-container"
                className="flex flex-row justify-between items-center gap-y-1 focus-within:border-checkout-200 rounded-lg border-gray-300 border py-2 px-3 w-full h-12"
              >
                <div className="flex flex-col justify-center w-full">
                  ...
                  <input
                    ...
                    onInput={(e) => {
                      ...
                      document
                        .querySelector("#email-container")
                        ?.classList.add("border-gray-300", "border");
                      document
                        .querySelector("#email-container")
              ...
              <p
                id="email-error"
                className="text-red-600 text-sm font-sans hidden"
              >
                Enter a valid email
              </p>
            </div>
          ...
          <section className="w-full">
            ...
            {loader && (
              <div className="trailing-progress-bar">
                <div className="trailing-progress animate-loading"></div>
              </div>
            )}
            {Object.values(cartItemObj.current!).map(
              (item: any, i: number) =>
                    ...
                    <section className="flex md:flex-row flex-col justify-between md:items-center items-start w-full gap-y-4">
                      ...
                      >
                        <div className="flex flex-row md:gap-x-7 gap-x-3 items-start">
                          <Image
                            src={
                              Object.values(frontBase64ImagesObj.current)[
                                i
                              ]![0]!
                            }
                            width={100}
          ...
          <footer className="w-full flex md:items-end flex-col font-sans gap-y-5 items-stretch">
            ...
            >
              {isCreatingCheckout ? (
                <div className="border-2 border-transparent rounded-full border-t-white border-r-white w-[15px] h-[15px] spin animate-spin"></div>
              ) : (
                "CHECKOUT"
              )}
            </button>
