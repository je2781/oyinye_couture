...
export default function Checkout({ cartItems, total, orderId, country, userEmail, userId, billingInfo, shippingInfo, saveBillingInfo, saveShippingInfo, csrf}: any) {
  ...
  async function completeOrder() {
    ...
    try {
      ...
      const userRes = await axios.patch(`/api/users/${userId}`, {
        firstName,
        lastName,
      ...
      if(userRes.status === 201 && userRes.data.emailJob){
          ...
          await qstashClient.publishJSON({
            ...
            body: {
                email: userRes.data.emailJob.user.email,
                userId: userRes.data.emailJob.user.id,
            },
            headers: {
              "x-internal-qstash-key": process.env.NEXT_PUBLIC_QSTASH_INTERNAL_KEY!
            }
        ...
        <section className="flex md:w-[55%] w-full flex-col md:h-full gap-y-9 border bg-white border-gray-300 border-l-0 border-t-0 border-b-0 pb-5 pt-7 md:pl-28 md:pr-9 px-7">
          <div className="flex flex-col gap-y-3">
            ...
            <div className="flex flex-col justify-center gap-y-1 focus-within:border-checkout-200 rounded-lg border-gray-300 border py-2 px-3 w-full h-14">
              <label htmlFor="email" className="text-xs animate-hideLabel text-gray-500">
                Email
              </label>
              <input
                placeholder="Email"
          ...
          >
            {loader ?  <div className="loader animate-spin" ></div> : <span>Complete order</span>}
          </button>
