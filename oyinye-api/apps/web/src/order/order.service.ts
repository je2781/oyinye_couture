    ...
    try {
      ...
      if (action) {
        if (action === "transaction-status") {
          ...
          if (cartId) {
            const transRes = await lastValueFrom(
              this.httpService.get(
                `https://webpay.interswitchng.com/collections/api/v1/gettransaction.json?merchantcode=${merchant_code}&transactionreference=${txn_ref}&amount=${amount}`
              )
            );

            if (
              transRes.data.ResponseCode === "10" ||
              transRes.data.ResponseCode === "11" ||
              transRes.data.ResponseCode === "00"
            ) {
              ...
              if (order) {
                ...
                await lastValueFrom(
                  this.adminClient.emit("cart_destroyed", {
                    id: cartId,
                    access_token: req.cookies['access_token']
                  })
                );
                ...
                const nextResponse = res.status(201).json({
            ...
            throw new BadRequestException(
              "payment not successful. invalid cart id"
            );
          }
        }
      } else {
