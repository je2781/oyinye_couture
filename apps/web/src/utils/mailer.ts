...
import { initializeSequelize } from "../db/connection";
...
const resetPasswordEmailData = async (
  ...
  //preparing email template and its data
    const templatePath = path.join(
      process.cwd(),
       'src/templates/notification.ejs'
    );
  ...
  const ejsData = {
    ...
    link: `${process.env.WEB_DOMAIN!}/resetpassword?token=${resetPasswordToken}`,
...
const reminderData = async (
  ...
  const message =
    password ? `Use ${password} to login into your account and access restricted content. If you did not register your details with us, you can ignore and delete this email.`: body;
  

  //preparing email template and its data
  const templatePath = path.join(
    process.cwd(),
    'src/templates/reminder.ejs'
  );
...
const verifyData = async (
  ...
  const ejsData = {
    ...
    isNewPass: false,
    isReset: false,
    link: `${process.env.WEB_DOMAIN!}/verifyemail/${emailType === EmailType.verify_account ? 'account' : emailType === EmailType.verify_reviewer ? 'reviewer' : 'buyer'}?token=${verifyAccountToken}`,
    year: new Date().getFullYear(),
  };
...
export const sendMail = async ({
  ...
  try {
    ...
    if (
      ...
      if(user){
        user.verify_token = hashedToken;
        user.verify_token_expiry_date = new Date(Date.now() + 3600000);

        await user.save();
      }
      htmlContent = await verifyData(hashedToken, emailType);
      ...
      htmlContent = await resetPasswordEmailData(hashedToken);
    }
