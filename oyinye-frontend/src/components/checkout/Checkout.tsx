  ...
  async function completeOrder() {
    ...
    try {
      ...
      if(paymentMethod === 'streetzwyze'){
        window.location.href = `https://wa.me/+2349061681807?text=I%20want%20to%20make%20a%20purchase%20on%20your%20website%20for%20item(s)%20with%20id%20${orderId}`;
      }else{
        window.checkout(
          async (response: any) => {
            if (response.desc) {
    ...
    <>
      ...
      <main className="min-h-screen w-full mx-auto container flex md:flex-row flex-col">
        ...
        <section className="flex md:w-[55%] w-full flex-col md:h-full gap-y-9 border bg-white border-gray-300 border-l-0 border-t-0 border-b-0 pb-5 pt-7 md:pl-28 md:pr-9 px-7">
          ...
          >
            {loader ?  <div className="loader spin" ></div> : <span>Complete order</span>}
          </button>
