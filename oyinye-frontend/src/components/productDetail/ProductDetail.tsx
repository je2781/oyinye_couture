...
const ProductDetail = ({
  ...
  const [selectedSize, setSelectedSize] = useState<string>(
    productSizes
      .find((size: any) => size.variant_id === paramsId)
      .number.toString()
  );
  ...
  React.useEffect(() => {
    async function setViewedProductsCookie() {
      api.get(
        `${process.env.NEXT_PUBLIC_WEB_DOMAIN}/api/products/cookie/${paramsId}`
      );
    }
            ...
            if (color.sizes.slice(1).length > 0) {
              for (let size of color.sizes.slice(1)) {
                sizeEls[
                  sizeEls.findIndex(
                    (el) =>
                      el.innerText.split(" ")[1] === size.number.toString()
                  )
                ]!.style.setProperty("color", "rgb(75, 85, 99)");
              }
            }
            //updating active dress size and pathname of current route
            const extractedColor = productColors.find(
              (color: any) =>
                color.name ===
                activeColorEl.innerText.charAt(0).toLowerCase() +
                  activeColorEl.innerText.slice(1)
            );
            const extractedSize = extractedColor.sizes.find(
              (size: any) => size.number === color.sizes[0].number
            );
            history.pushState(
              null,
              "",
              `/products/${productTitle.replace(
                " ",
                "-"
              ).toLowerCase()}/${extractedColor.name.replace(" ", "-").toLowerCase()}/${
                extractedSize?.variant_id
              }`
            );
  ...
  function handleSizeChange(e: React.MouseEvent) {
    ...
    const extractedSize = extractedColor.sizes.find(
      (size: any) =>
        size.number === parseInt(activeSizeEl.innerText.split(" ")[1]!)
    );
    history.pushState(
      null,
      "",
      `/products/${productTitle
        .replace(" ", "-")
        .toLowerCase()}/${selectedColor.toLowerCase().replace(" ", "-")}/${
        extractedSize?.variant_id
      }`
    );
  ...
  const mainContent = (
    <main className="container mx-auto bg-white lg:gap-y-28 gap-y-14 w-full min-h-screen pl-2 pr-3 lg:pl-0 lg:pr-6 lg:pt-0 pt-8 flex flex-col">
      <section className="flex lg:flex-row flex-col gap-y-7">
        <article
          className={`flex flex-col gap-y-2 w-full lg:w-[46%] ${
            articleIsNotSticky ? "" : "lg:sticky lg:top-0"
          } lg:h-full lg:pt-8`}
        >
          <Swiper
            ...
            pagination={{
              ...
              renderBullet: (index, className) => {
                ...
                return `<span class="${className}" style="background-image: url(${images[
                  index
                ].replace(
                  "/app/public",
        ...
        <section className=" w-full lg:px-12 px-6 lg:w-[54%] lg:h-full lg:pt-8 flex flex-col gap-y-5">
          ...
          <div className="flex flex-col items-start gap-y-2" id="color-list">
          ...
          <div className="mt-2 flex flex-col gap-y-3 xl:w-[80%] w-full">
            {toastError && (
              <div className="flex flex-row gap-x-2 text-sm font-sans items-center">
                <i className="fa-solid fa-circle-exclamation text-red-600"></i>
                <p className="text-gray-400">
                  You can&apos;t add more {productTitle} to the cart
                </p>
              </div>
            )}
            <button
              className="font-sans xl:px-44 px-28 py-2 ring-gray-600 hover:ring-1 border border-gray-600 text-gray-600 flex flex-row justify-center items-center"
              onClick={() => {
                if (
                  items.some(
                    (item: any) =>
            ...
            <button
              onClick={async () => {
                try {
                  setIsBuyingNow(true);
                  const cartPostRes = await api.post(
                  ...
                  const cartUpdateRes = await api.patch(
                  ...
                  router.push(
                    `/checkouts/cn/${checkoutRes.data.checkout_session_token}`
                  );
                } catch (error) {
                  const e = error as Error;
                  toast.error(e.message);
              ...
              className="font-sans xl:px-44 px-28 py-2 ring-[#5a31f4] hover:bg-[#512bd8] hover:ring-1 bg-[#5a31f4] text-white"
            >
              {isBuyingNow ? (
                <div className="loader spin"></div>
              ) : (
              ...
              >
                <h1 className="flex flex-row gap-x-3">
                  <Image
                    ...
                    alt="ruler"
                    role="presentation"
                    className="w-6 lg:w-[28px]"
                    width={28}
                    height={28}
                  />
                  <span className="text-gray-500 font-sans lg:text-lg text-[1rem]">
                    Size Chart
                  </span>
                </h1>
                <i
                  className={`fa-angle-down fa-solid text-sm text-gray-500`}
                ></i>
              </header>
              <div
                id="sizes-content"
                className="border border-gray-200 pt-3 pb-6 px-5 lg:ml-3 lg:mr-12 mr-5 flex-col gap-y-4 hidden"
              >
                ...
                <div className="flex flex-col items-start w-full gap-y-1">
                  <header className="w-full flex flex-col gap-y-5">
                    <h5 className="text-[.5rem] font-sans font-thin pl-1">
                      WWW.OYINYE.COM
                    </h5>
                    <table className="w-full border-separate border-spacing-x-1">
                      <thead>
                        <tr>
                          ...
                          <th className="text-[.7rem] text-white text-center bg-black w-[11.5%] py-1">
                            XS
                          </th>
                          <th className="text-[.7rem] text-white text-center bg-black w-[11.5%] py-1">
                            S
                  ...
                  <table className="w-full border-separate border-spacing-1">
                    <tbody>
                      <tr>
                        <td className="text-[.7rem] text-black text-center bg-transparent border border-gray-400 w-[31%] font-bold">
                          BUST
                        </td>
                        <td className="text-[.7rem] text-black text-center bg-transparent border border-gray-400 w-[11.5%] py-1 font-medium">
                          33
                        </td>
                        <td className="text-[.7rem] text-black text-center bg-transparent border border-gray-400 w-[11.5%] py-1 font-medium">
                          35
            ...
            <div>
              <header
                onClick={(e) => {
                  let downAngle = e.currentTarget.querySelector("header i");
                  let header = e.currentTarget;
                  let content = header.parentNode?.querySelector(
                    "#shipping-content"
                  );
